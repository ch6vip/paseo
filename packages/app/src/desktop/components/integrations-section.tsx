...
export function IntegrationsSection() {
  ...
  const handleUninstallSkills = useCallback(async () => {
    ...
    const confirmed = await confirmDialog({
      title: t("settings.integrations.uninstallSkills.title"),
      message: t("settings.integrations.uninstallSkills.body"),
      confirmLabel: t("settings.integrations.uninstallSkills.confirm"),
      destructive: true,
    });
    if (!confirmed) return;
  ...
  return (
    <SettingsSection title={t("settings.integrations")} trailing={trailing}>
      <View style={settingsStyles.card}>
        <View style={settingsStyles.row}>
          <View style={settingsStyles.rowContent}>
            ...
            <Text style={settingsStyles.rowHint}>
              {t("settings.integrations.commandLine.hint")}
            </Text>
          </View>
          {cliStatus?.installed ? (
            <View style={styles.installedLabel}>
              <Check size={14} color={theme.colors.foregroundMuted} />
              <Text style={styles.mutedText}>{t("common.installed")}</Text>
            </View>
          ) : (
            <Button
              variant="outline"
        ...
        <View style={ROW_WITH_BORDER_STYLE}>
          <View style={settingsStyles.rowContent}>
            ...
            <Text style={settingsStyles.rowHint}>
              {skillsState === "drift"
                ? t("settings.integrations.updateAvailable")
                : t("settings.integrations.skillsHint")}
            </Text>
          </View>
          <SkillsActions
            state={skillsState}
    ...
    return (
      <View style={styles.actionsRow}>
        ...
        <Button variant="outline" size="sm" onPress={onUninstall} disabled={isWorking}>
          {t("settings.integrations.uninstallSkills.confirm")}
        </Button>
      </View>
    );
  }
    ...
    return (
      <View style={styles.actionsRow}>
        <Button variant="outline" size="sm" onPress={onUpdate} disabled={isWorking}>
          {isWorking ? t("settings.integrations.working") : t("settings.integrations.update")}
        </Button>
        <Button variant="outline" size="sm" onPress={onUninstall} disabled={isWorking}>
          {t("settings.integrations.uninstallSkills.confirm")}
        </Button>
